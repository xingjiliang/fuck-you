...
class Model:
    ...
    def __init__(self, global_config, sample, info_input_embeddings, is_training):
        ...
        self.wide_part_bias = tf.get_variable("wide_part_bias",
                                              [1],
                                              tf.float32,
                                              initializer=tf.random_normal_initializer)
        self.wide_part_logits = tf.add(tf.matmul(self.wide_part_vector, self.wide_part_weights), self.wide_part_bias)
        ...
        # 这里直接reduce_sum即可
        self.logits = tf.reduce_sum([temp_hidden_vector, self.wide_part_logits], 0)
